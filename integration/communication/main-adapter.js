...
class MainAdapter {
    ...
    waitForPanelsAndSendInitialState() {
        // Wait for iframes to fully load and UIAdapter to initialize
        // Panels typically take ~1000ms to load
        setTimeout(() => {
    ...
    handleHierarchyEvent(event) {
        ...
        // If this is a layout property change (autoLayout.*) for a selected object,
        // also send updated object data so property panel can refresh
        if (changeData?.type === 'layout-property-changed' && objectId) {
            const selectedObjects = this.selectionController?.getSelectedObjects() || [];
            const selectedIds = selectedObjects.map(obj => {
                const objectData = this.sceneController.getObject(obj.id || obj.uuid);
                return objectData?.id;
            }).filter(Boolean);

            // Check if the changed object is currently selected
            if (selectedIds.includes(objectId)) {
                // Re-send selection with updated data
                const allObjectData = selectedIds.map(id => this.getObjectDataForUI(id)).filter(Boolean);
                const selectionMessage = window.MessageProtocol.MessageBuilders.selectionChanged(
                    selectedIds,
                    allObjectData.length > 0 ? allObjectData[0] : null
                );
                selectionMessage.payload.allObjectData = allObjectData;
                selectionMessage.strategy = window.MessageProtocol.EMISSION_STRATEGY.IMMEDIATE;
                this.send(selectionMessage);
            }
        }
    }
    ...
    handleSelectionEvent(event) {
        ...
        // Get full object data for ALL selected objects (for multi-selection support)
        const allObjectData = selectedObjectIds.map(id => this.getObjectDataForUI(id)).filter(Boolean);

        // For backward compatibility, also send single objectData (first object)
        const objectData = allObjectData.length > 0 ? allObjectData[0] : null;

        // Build and send selection message
        const message = window.MessageProtocol.MessageBuilders.selectionChanged(
            selectedObjectIds,
            objectData
        );

        // Add allObjectData to payload for multi-selection
        message.payload.allObjectData = allObjectData;
